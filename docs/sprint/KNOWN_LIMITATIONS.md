...
- `smoke_desktop.py` still targets pre-OmniCam control names and is **not** a gate. Its replacement is `dotnet/tests/ui-smoke` (PowerShell + UIA: demo → nest → CAM → export with file assertions, stale banner). It runs locally via `run-all.ps1` and as a **blocking** step of the Windows CI job (made blocking 2026-09-03 after five consecutive green runs on the hosted runner). A red UI smoke fails the job; the screenshots are uploaded as the `ui-smoke-screenshots` artifact — read them before changing a scenario.
...
- `OMNICAM_LIBRARY_PATH` (environment variable) relocates `library.json`; the UI smoke uses a throw-away copy per scenario so it never touches the operator's library.
- `library.json` is written atomically (`.tmp` → swap, previous good copy kept as `library.json.bak`). If the main file is truncated or unparsable at startup the backup is loaded and a warning toast says so ("参数库已从备份恢复"); only when both are unusable does the app fall back to factory defaults — and then it says that too ("参数库已重置为出厂默认"), instead of silently losing the remnant library.
- Display layers (纹理 / 特征 / 标签锚点 / 尺寸 / 快移线) persist in `library.json` (`display`), so a hidden layer stays hidden after a restart — the 显示 ▾ menu shows the current state. 帮助 → 快捷键 opens a grouped sheet generated from `Desktop.Core.ShortcutCatalog`; a test checks the window still handles every key it advertises.
