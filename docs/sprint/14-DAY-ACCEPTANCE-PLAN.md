...
> 实现快照：`sprint/14d-rc` · `rc-14d-audit2-20260805` · `RC_REPORT.md` / `KNOWN_LIMITATIONS.md`。
...
> **验收核对：** 2026-08-12 · `dotnet test -c Release` → Domain 63 / Package 16 / Infra 2 全绿。  
> 图例：PASS = 自动/文档证据充分 · PARTIAL = 部分满足或仅手工 · FAIL/PENDING = 未过或未做。详见附录 D。

| # | 模块 | 需求状态 | 优先级 | 实现大致 | 验收 | 依赖 |
|---|------|----------|--------|----------|------|------|
| 1 | 产品范围与非目标 | 草稿 | P0 | 文档层 | PARTIAL 3/4 | — |
| 2 | 数据合同 | 草稿 | P0 | 已交付核心 | PASS 4/4 | — |
| 3 | 导入与校验 | 草稿 | P0 | 已交付核心 | PARTIAL 3/4 | 2 |
| 4 | 板件几何与特征编辑 | 草稿 | P1 | 已交付核心 | PASS 3/3 | 2 |
| 5 | Undo / 脏标记 / 会话 | 草稿 | P0 | 已交付核心 | PARTIAL 2/3 | 4 |
| 6 | 密排与分组 | 草稿 | P0 | 已交付 BLF | PASS 4/4 | 2、3 |
| 7 | Nest 导出硬门 | 草稿 | P0 | 已交付 | PASS 3/3 | 6 |
| 8 | 刀具与工艺绑定 | 草稿 | P0 | 已交付 | PASS 3/3 | 2 |
| 9 | CAM 安全 / Preflight | 草稿 | P0 | 已交付 | PASS 4/4 | 8、10 |
| 10 | 刀路（Pocket/Groove/Drill/Contour） | 草稿 | P0 | 已交付 v1 | PASS 4/4 | 8、9 |
| 11 | 双面与定位 A/B | 草稿 | P1 | **Partial** | PASS 当前 2/2；完整版 0/2 | 2、9、12 |
| 12 | 后置与导出 Bundle | 草稿 | P0 | 已交付 Sheet×Tool | PARTIAL 3/4 | 7–10 |
| 13 | 标签 / BOM / 工单 | 草稿 | P1 | 已交付核心 | PARTIAL 1/2 | 2、12 |
| 14 | Desktop UI 与生产流程 | 草稿 | P1 | 已交付主路径 | PENDING 0/3 | 3–13 |
| 15 | ComputeWorker 一致性 | 草稿 | P1 | 已交付核心 | PARTIAL 1/2 | 6、12 |
| 16 | 测试 / Smoke / 发布 | 草稿 | P0 | 自动绿；人工待签 | PARTIAL 1/3 | 全部 |
...
- **需求状态：** 草稿  
- **现状摘要：** 条件 RC：单面、混材料/混厚、Nest→CAM→Sheet×Tool 导出；未宣称 NFP / 生产双面 WCS / 仿真 / 签名包 / 自动 M6。

### 目标
成为**独立切割站**：上游 CAD/Fusion 只导出 `cabinetnc.woodjob`（或 legacy cut-package）；本产品完成编辑、密排、安全 CAM、可追踪导出，使车间能按 Bundle 空跑/加工，且错误深度/顺序/缺刀/碰撞会挡导出。

### 必须有
1. 真实 woodjob / cut-package → 可编辑 2.5D 工件  
...
- [ ] Campaign 2 是否以「真 NFP」还是「双面 WCS」为第一优先  

---

## 2. 数据合同（Workpiece / Package / Woodjob）

- **需求状态：** 草稿  
- **现状摘要：** 运行时仍为扁平 `CutPackage.Panels[]`；Project/Module/Workpiece 为软身份字段；详见 `workpiece-contract.md`。

### 目标
制造身份与朝向进入正式模型，不靠 SVG 猜几何；legacy cut-package 仍可导入。

### 必须有
| 字段 | 要求 |
|------|------|
| `PanelId` / `Identity.WorkpieceId` | 必有 |
| `ThicknessMm` | 必有且 >0（或可经 materials 解析） |
| `Material` | 强烈建议；分组 Nest 依赖 |
| `Outline` ≥3 点 | 必有 |
| `Orientation`：Primary/Milling Face、Grain、AllowedRotations、AllowMirror | 可选但 round-trip |
| `Side` / A·B 占位 | 可选 |
| `EdgeBanding` / `Notes` | 可选 |
| Features：hole / groove / pocket / cutout | 有则带 Depth 等工艺字段 |

### 不要做
- 强制以 SVG 为生产几何权威  
- 本阶段强制 UI 改造成 Project→Module 树  
- 未经批准的 woodjob breaking schema  

### 验收标准
- [x] 新字段序列化 round-trip 测试通过 — **PASS**（WoodJob/ProjectSession）  
- [x] 缺厚度 → 导入错误 `thickness` — **PASS**  
- [x] legacy cut-package 绿灯 — **PASS**（CutPackageImporter）  
- [x] 文档 `workpiece-contract.md` 与实现一致 — **PASS**（核心字段对齐）  

### 优先级 / 依赖
P0；被 3/4/6/11/13 依赖。

### 待拍板
- [ ] schema bump 策略：仅 additive vs 允许 breaking（默认 **additive**）  
...
- **需求状态：** 草稿  
- **现状摘要：** woodjob zip/目录、cut-package JSON；SHA-256 checksum；材料表解析；样本 120 板 + 小 JSON。

### 目标
一键载入车间包；坏包**失败可见**，不得静默污染当前作业。

### 必须有
1. 支持 `cabinetnc.woodjob` v2 zip/文件夹  
2. 支持 legacy `cabinetnc.cut-package` JSON  
3. checksum 不匹配 → 失败，明确 SHA-256 mismatch  
4. 空 panels / 非法包 → 失败对话框（路径+错误列表）  
5. 导入成功摘要：格式、JobId、板件数、材料/板材规格数、警告列表  
6. Desktop「打开示例」指向 `demo_woodjob_120.zip`  
...
- **需求状态：** 草稿  
- **现状摘要：** Feature Inspector；孔/槽可改；镜像/剪贴板；小板警告；DXF 导入矩形/折线级。
...
- **需求状态：** 草稿  
- **现状摘要：** 三类 undo；ManufacturingDirty；Sqlite 工程保存/恢复。
...
- **需求状态：** 草稿  
- **现状摘要：** 材料+厚度分组 BLF（AABB）；高级 stub→`blf_fallback`；缺陷区 AABB punch；补板可入排；非 NFP。
...
- **需求状态：** 草稿  
- **现状摘要：** `NestExportGate`：间距、碰撞、混组等错误挡导出。
...
- **需求状态：** 草稿  
- **现状摘要：** Pocket：inset+分段 zigzag+finish；段间 G0；Drill peck；Contour stepdown；刀补 Clipper。
...
- **需求状态：** 草稿（实现 **Partial**）  
- **现状摘要：** 无 `FaceRegistration.Strategy` 时 B 面 `no_registration`；无生产翻板 WCS 文件对。

### 目标（完整版，待拍板后实施）
B 面加工仅在定位策略明确后允许；导出成对程序与原点约定可执行。

### 必须有（当前最低）
1. 存在 B 面工序且无 registration → **硬挡导出**  
2. 文档与 UI 不得暗示「已支持生产双面」  
...
- **需求状态：** 草稿  
- **现状摘要：** `{job}_S{n}_{Tn}.nc` + `{job}_S{n}.dxf` + manifest；单刀头含直径与 Feed/RPM；无 M6。
...
8. 方言：generic（M2）与 Fanuc-like（M30）可选  
...
### 待拍板
- [ ] 默认方言：`generic` vs `fanuc_like`（原计划偏 Fanuc-like）  
- [ ] 命名是否允许客户模板  
...
- **需求状态：** 草稿  
- **现状摘要：** 七模块壳 + 五步生产；导入→板材设备→密排→刀路→导出；CAM playhead 有基础。
...
- **需求状态：** 草稿  
- **现状摘要：** Worker 走 NestEngineRouter；Desktop 多数本地 Domain；有 parity 测试。

### 目标
长计算算法权威在 Domain（可被 Worker 调用）；禁止 UI 私有分叉。

### 必须有
1. Nest 权威：`GroupedBlfNester` / `NestEngineRouter`  
2. 同输入 parity 测试保持绿  
...
- **需求状态：** 草稿  
- **现状摘要：** 自动测试绿；UIA agent 易挂；人工 Smoke / 机床清单待签；PR #1 未合 main。
...
3. 回归覆盖：多材料、混厚、多 Sheet×Tool、Pocket 安全、顺序、槽深、120 应力、parity  
...
### 验收标准
- [x] 自动套件绿有日志 — **PASS**（Domain 63 / Package 16 / Infra 2）  
- [ ] Smoke 清单无未解释的 MANUAL PENDING — **PENDING**（10/10 仍 MANUAL PENDING）  
- [x] tag 指向含审计修复的 commit — **PASS**（`rc-14d-audit2-20260805` → `bab9103`）  
...
1. 对外口径是否确认「条件 RC · 单面」  
2. T1/T2/T3 与店内刀号映射  
3. ThroughAllowance=0.5 / Spoil=1.0 是否冻结  
4. 默认 NC 方言  
5. 合 main 是否强制空跑签字  
6. 双面：翻板轴 / 原点 / 策略（阻塞模块 11 完整版）  
7. 是否启动真 NFP（建议 Later）  
8. 是否做 M6（建议等样例）  

---

## 附录 D — 验收核对记录（2026-08-12）

**证据命令：** `dotnet test -c Release` → Domain **63** / Package **16** / Infra **2** 全绿。  
**分支/tag：** `sprint/14d-rc` · `rc-14d-audit2-20260805`（`bab9103`）。

### 汇总

| 结果 | 模块数（按模块验收列） |
|------|------------------------|
| 全 PASS | 2, 4, 6, 7, 8, 9, 10（及 11 当前最低） |
| PARTIAL | 1, 3, 5, 12, 13, 15, 16 |
| PENDING | 14（几乎全靠人工）；11 完整版；合 main |

**粗算可自动验收条目：** 约 **35/45** PASS；**~6** PARTIAL；**~8** PENDING（含人工/未来双面）。
...
| Desktop 脏导出 / 五步 / Smoke | 签 `MANUAL_SMOKE_10MIN.md` |
...
- **制造主链（Nest/CAM/安全/Sheet×Tool）自动验收基本达标。**  
- **发布级验收未达标：** Desktop 人工 Smoke、上机、合 main 门禁仍开着。  
- **不要**把当前状态标成「全部验收通过」。
